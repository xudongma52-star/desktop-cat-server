...
文档版本：v0.8
更新日期：2026-09-16
阶段：个人文章 CRUD、文章温馨回忆轮播和 Windows 桌面猫已可运行；图片回忆、提醒与 RAG 保持规划状态。

本文区分当前已实现部分与后续规划。用户已明确选择 Spring Boot 3、JDK 21 和原生 MyBatis。本次实际运行环境为 Oracle GraalVM JDK 21.0.12、Node.js 24.16.0、pnpm 11.19.0、Maven 3.9.15，数据库集成验证使用 PostgreSQL 16.13。启动方式与当前范围见 README.md；个人文章数据库业务已经实现，Python AI 服务和云端部署仍未实施。
...
桌面端当前用 JSON 保存窗口位置。后续少量偏好、近期提醒及展示记录仍由主进程统一读写，并采用临时文件替换方式减少写入中断造成的数据损坏。照片、动画和声音使用普通文件缓存。完整离线编辑尚未纳入第一版，不提前引入本地数据库。
...
| Flyway | 由 Boot 3.5.16 管理 | 数据库版本迁移；已执行 V1、V2 |
...
当前 `postgres` 配置连接 PostgreSQL，并由 Flyway 管理 `cat_profile` 与 `personal_record`；`local` 配置关闭数据源自动配置，用于不依赖数据库的基础开发。小猫资料与个人文章都已使用 MyBatis 接口和 XML 完成真实数据库读写。
...
- 个人文章模块的请求和返回对象统一放在模块级 `record.dto`，类名统一以 `Dto` 结尾；Controller 直接把 DTO 交给 Service，不维护内容重复的接口 DTO 和业务 DTO；Service Impl 负责业务编排、状态校验、事务以及 DTO 与数据库对象之间的转换；数据库对象 `PersonalRecordDO` 单独放在 `dao.dataobject`，DAO 接口留在 `dao`，SQL 统一放 Mapper XML。
...
记录与文章回忆模块已经实现；待办提醒、用户设置、文件、AI 编排和鉴权仍按后续需求设计。
...
当前验证范围包括网页类型检查与生产构建、后端自动化测试与打包、Flyway 全新数据库迁移、MyBatis XML 真实读写、REST 接口，以及桌面端类型检查、构建、Windows NSIS 打包和进程启动。活动系统已覆盖接受、拒绝、随机时长、真实窗口位移和选择面板渲染。Python 索引与引用、图片回忆、提醒去重和睡眠恢复要在对应功能实现后验证。
...
2026-09-16 验证结果：个人文章网页生产构建、桌面端类型检查与生产构建通过；Maven Wrapper verify 通过 7 项后端测试并成功打包。在全新 PostgreSQL 数据库上，Flyway V1、V2 执行成功；真实 HTTP 请求已覆盖文章创建、详情、摘要分页与类型筛选、更新、文章回忆、旧版本冲突、逻辑删除和删除后不可见，确认 MyBatis XML 与 PostgreSQL 链路可用。桌面宽度与 390px 窄屏的首页、列表、创建、详情及回忆轮播均通过浏览器检查，控制台无错误。
...
后端同样采用 Controller → Service 接口 → Service Impl → DAO 接口 + XML。文章模块统一使用 `record.dto` 在 Controller 和 Service 之间传递数据；`dao.dataobject.PersonalRecordDO` 只负责数据库字段映射，不会进入 Controller 或 Service 接口。请求字段、响应示例和错误码见 [接口文档](./接口文档.md)。当前文章接口为：
...
当前实施进度：已完成前后端框架与联通 → 已完成桌面猫陪伴 → 已完成小猫资料管理与桌面同步 → 已完成个人文章 CRUD 与文字回忆轮播。下一阶段可根据真实使用频率，在待办提醒、图片回忆和带引用的 RAG 问答之间确定优先级。
