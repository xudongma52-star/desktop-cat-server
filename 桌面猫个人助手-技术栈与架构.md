...
文档版本：v0.7
更新日期：2026-09-14
阶段：网页前后端起步框架与 Windows 桌面猫活动系统已创建，其余功能保持规划状态。

本文区分当前已实现部分与后续规划。用户已明确选择 Spring Boot 3、JDK 21 和原生 MyBatis。本次实际运行环境为 Oracle GraalVM JDK 21.0.12、Node.js 24.16.0、pnpm 11.19.0、Maven 3.9.15。启动方式与当前范围见 README.md；Python、数据库业务和云端部署仍未实施。
...
- 当前实现发呆、睡觉、舔毛、玩耍、散步和奔跑六种状态。每项活动按自身范围随机持续 1–10 分钟，结束后自主选择不同的下一项；用户单击小猫打开活动面板，请求有按状态配置的拒绝概率和对应反馈。
...
- 当前六种状态复用奔跑和睡觉两张素材，并通过不同 CSS 动画、毛球、脚印、星光、速度线和睡眠符号表达。后续角色包可增加各活动专属透明 PNG 动作帧或序列图；首批待补动作是舔毛、玩耍、散步和进食。
- 摸头、喂食与基础动画均设计为在本地完成，不依赖模型请求；当前先实现摸头反馈与基础动画。
...
当前框架的验证范围是网页类型检查与构建、默认无数据库模式的后端测试、真实 HTTP 请求与开发代理，以及桌面端类型检查、构建、Windows NSIS 打包和进程启动。活动系统已覆盖接受、拒绝、随机时长、倒计时、真实窗口位移和选择面板渲染验证；MyBatis 数据库读写、数据库迁移、Python 索引与引用、提醒去重和睡眠恢复将在对应功能实现后验证。

2026-09-14 验证结果：pnpm build 通过，Maven Wrapper verify 通过（1 项集成测试），JAR 成功打包；浏览器经 Vite 代理成功请求 Java 状态接口，刷新连接成功。桌面端类型检查、electron-vite 生产构建和 electron-builder NSIS 打包通过；0.3.0 成品成功启动，奔跑测试在 2.5 秒内移动约 131 像素，并观察到接受与拒绝两种返回。PostgreSQL profile 仅完成配置预留。

### 首条业务链路（小猫资料）

`cat_profile` 使用 `profile_id` 作为明确主键，`profile_key` 标识唯一主资料，`version` 提供乐观锁，`created_at` 与 `updated_at` 保留审计时间。网页通过 Java REST 接口读取和修改名字；Java 使用 Service 校验、Repository 抽象与原生 MyBatis XML；Electron 每 10 秒同步，并在用户目录缓存最近成功资料。
...
当前实施顺序：已完成前后端框架与联通 → 已完成小猫展示、活动选择与自主移动 → 已完成小猫名字管理与桌面同步 → 下一步完成一条记录的保存和读取 → 提醒与回忆陪伴 → 带引用的 RAG 问答 → 按真实使用需求扩展。
