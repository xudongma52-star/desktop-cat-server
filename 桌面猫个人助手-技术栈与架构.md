...
本文同时作为长期产品构想和架构决策参考。文中标为“规划”“长期构想”或“预留”的能力只记录方向、边界和可能的接入位置，不构成下一轮开发任务，也不表示必须按书写顺序实现。每轮只选择一个范围清楚、能独立验证且确实改善使用体验的目标；开始实现前仍需结合真实使用反馈确认优先级、数据设计、隐私边界和验收标准。
...
DDD 在本项目中是理解和组织复杂业务的一种方法，不是要求立即重写现有代码的目标，也不通过增加 Entity、Repository、Domain Service 等名称来证明已经使用 DDD。当前 Controller → Service 接口 → Service Impl → DAO + XML 的分层继续作为简单 CRUD 和查询功能的默认实现。

当一个模块出现多条会持续演化、容易散落在多个 Service 中的业务规则时，再为该模块引入领域模型。例如，小猫的饱食度、连续喂食冷却和不同食物效果应由“小猫”领域对象维护；重复提醒的下一次触发、稍后提醒、错过恢复和完成限制应由“提醒”领域对象维护。应用服务负责编排一次操作，领域对象保护业务规则，DAO 仍由 MyBatis 接口和 XML 完成数据库访问。只有领域模型确实需要与持久化模型隔离时，才考虑增加领域 Repository 及其基础设施适配器；它不等同于 DAO Impl，也不能为简单转发而存在。

未来可以按业务语言识别以下限界上下文，但当前不据此批量建包或创建空类：

| 候选上下文 | 主要业务概念 | 适合考虑领域模型的时机 |
| --- | --- | --- |
| 陪伴 | 小猫资料、活动、喂食、心情、互动 | 出现饱食度、冷却、行为约束和状态联动时 |
| 内容 | 日记、心得、回忆、附件、AI 使用授权 | 内容版本、发布状态和授权规则开始组合变化时 |
| 提醒 | 待办、触发规则、稍后提醒、完成、错过恢复 | 从一次性提醒扩展到重复与离线恢复规则时 |
| 知识库 | 索引任务、内容切片、检索范围、引用 | 开始实现可重建索引和带来源回答时 |

开发时优先保持统一业务语言，例如固定使用“当天内心”“提醒”“稍后提醒”“允许 AI 使用”“回忆轮播”等用户能够理解的词。简单功能继续直接实现，复杂规则出现后再逐步提取；不同上下文不共享一个不断膨胀的通用业务对象。
...
以下能力用于记录希望产品最终提供的体验。它们是候选方向，不是下一轮必须完成的任务；没有进入某轮明确范围前，不提前创建数据库表、接口、空模块或抽象层。

### 当天情绪与适时关心

- 大模型只读取用户主动记录且明确允许本功能使用的当天情绪内容，结合当天多次记录观察变化，在合适的时间生成简短、自然的关心话语。
- 不在每次输入后立即生成安慰或固定套话。是否生成、生成时机和当天展示次数需要单独设计，优先避免打扰和情绪记录被“系统回应”占满。
- 网站在“当天内心”旁展示小猫对当天状态的简短分析，说明依据来自哪些当天记录；内容使用日常、克制的表达，不进行心理诊断，不使用疾病标签，也不替代专业帮助。
- 用户可以关闭分析和主动关心；原始情绪内容不写入日志。模型失败时保留原始记录功能，不阻塞保存和查询。

### 带来源引用的个人文章 RAG

- 只检索用户通过 `rag_enabled` 明确允许 AI 使用且当前有效的内容，未授权、已删除或已关闭授权的内容不得进入回答上下文。
- 回答展示来源文章、相关片段和可跳回的原文位置；没有足够依据时直接说明未找到，不用模型常识冒充个人记忆。
- 原文是业务事实，切片、向量和摘要属于可重建数据。文章更新、删除或撤销授权后，旧索引不得继续参与检索。

### 待办与桌面小猫提醒

- 在现有创建、修改、完成和一次性到期提醒基础上，候选能力包括稍后提醒、重复规则和更完整的离线恢复。
- 桌面端继续使用版本去重；休眠、关机或断网后恢复时，需要区分已错过、已完成和仍应展示的提醒，避免重复打扰。
- 每次扩展先明确一条业务规则及其跨网站、Java 和 Electron 的完整行为，不一次性实现复杂日历系统。

### 回忆与桌面陪伴表现

- 增加用户主动选择的图片回忆、喜欢的话和动漫角色素材，并继续保留是否进入回忆轮播的独立控制。
- 扩展小猫动作、表情、主题和互动方式，让桌面组件在休息时提供陪伴；纯表现类交互优先本地完成，不为每个动作调用大模型。
- 使用动漫角色或其他外部素材前单独确认来源、授权和分发边界，个人原型中的预留不等于可以随安装包公开发布。

### 延后设计的长期能力

- 长期记忆、跨日情绪趋势和完整 LLM 对话需要在积累真实使用数据、明确实际价值和隐私边界后再设计。
- 默认不把每次聊天、每条情绪或每次桌面互动写入长期记忆。进入长期记忆应有明确触发、可查看来源、可编辑和可删除的控制。
- 跨日趋势只描述用户主动记录中的可观察变化，不推断疾病、人格或用户未表达的事实。

候选功能进入实施前，应先回答：它解决了哪一个真实使用问题、需要读取哪些数据、用户如何授权和撤销、失败时是否影响基础功能、怎样验证它确实减少负担或增加陪伴感。答案不清楚时继续保留为构想，不排入下一轮。
