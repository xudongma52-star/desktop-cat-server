...
- Controller 包内放请求和响应 DTO；Service Impl 负责业务编排、状态校验与事务；DAO 包内放数据库对象和 MyBatis 接口，SQL 统一放 Mapper XML。
...
后端同样采用 Controller → Service 接口 → Service Impl → DAO 接口 + XML。当前接口为：
