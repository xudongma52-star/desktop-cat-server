...
文档版本：v0.5
更新日期：2026-09-14
阶段：网页前后端起步框架与 Windows 桌面猫最小可执行单元已创建，其余功能保持规划状态。
...
- 当前最小单元使用代码内 SVG 小猫和 CSS 动画，实现呼吸、眨眼、摇尾巴与被摸后的跳动；这是可替换的原创占位形象。
- 当前猫叫由 Web Audio 在点击时本地合成，不读取麦克风，也不请求网络。正式角色阶段再把自己的小猫图片、动作配置与声音组织为角色包。
- 后续角色包可使用透明 PNG 动作帧或序列图；首批待补动作是进食和睡觉。
- 摸头、喂食与基础动画均设计为在本地完成，不依赖模型请求；当前先实现摸头反馈与基础动画。
- Electron 主进程负责窗口、文件、通知等系统能力；Vue 渲染进程通过 preload 暴露的明确方法与主进程通信，启用上下文隔离。
- 当前已实现透明区域的鼠标穿透、拖动、托盘显隐与退出、单实例运行和窗口位置记忆。
...
当前框架的验证范围是网页类型检查与构建、默认无数据库模式的后端测试、真实 HTTP 请求与开发代理，以及桌面端类型检查、构建、Windows NSIS 打包和进程启动。小猫的拖动、点击声音和托盘交互可在当前已启动的程序中人工体验；MyBatis 数据库读写、数据库迁移、Python 索引与引用、提醒去重和睡眠恢复将在对应功能实现后验证。

2026-09-14 验证结果：pnpm build 通过，Maven Wrapper verify 通过（1 项集成测试），JAR 成功打包；浏览器经 Vite 代理成功请求 Java 状态接口，刷新连接成功。桌面端类型检查、electron-vite 生产构建和 electron-builder NSIS 打包通过，解包后的成品成功启动并保持运行。PostgreSQL profile 仅完成配置预留。

当前实施顺序：已完成前后端框架与联通 → 已完成小猫最小展示与互动 → 下一步完成一条记录的保存和读取 → 提醒与回忆陪伴 → 带引用的 RAG 问答 → 按真实使用需求扩展。
