...
- 已完成可追溯的知识问答：Java 只读取当前用户最近 100 篇记录中主动开启 `rag_enabled` 的正文，Python FastAPI 使用中文字符级 TF-IDF、文本切片和余弦相似度返回最多 5 个相关片段，再由豆包 `Doubao-Seed-2.1-turbo` 仅根据命中片段生成带来源编号的回答。模型未配置或暂时不可用时自动降级为原文检索，不影响来源查看；当前尚未保存向量索引。
...
- 持久化向量索引、重复提醒、拖拽投喂冻干和开机自启尚未实现；相关接入位置作为后续扩展边界。
