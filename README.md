...
- 日记、数据库业务表、业务 Mapper、RAG、提醒、拖拽投喂冻干、登录和开机自启尚未实现；页面中的后续方向明确标注为待开发。
...
桌面猫不依赖 Java 后端即可运行。单击小猫会播放真实猫叫并打开活动面板，可以请求发呆、睡觉、舔毛、玩耍、吃冻干、散步或奔跑；小猫可能按活动设定接受或拒绝，活动结束后会自主选择下一项。玩耍、散步和奔跑时，窗口会在当前显示器可用区域内自由移动、随机转向并在屏幕边缘折返；打开选择面板或拖动小猫时会暂停自动移动。
