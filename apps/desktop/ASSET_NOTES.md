...
猫叫使用五段随应用分发的真实猫咪录音，来源和 CC0 许可证记录在 `THIRD_PARTY_NOTICES.md`。`src/renderer/src/cat-sounds.ts` 按打招呼、触摸、提醒、开心和撒娇抗议选择不同录音，并加入很轻的音高变化；连续互动时会快速淡出上一声。运行时只读取本地 WAV，不访问网络。
