...
小猫声音使用六段随应用分发的真实猫咪短颤音和咕噜招呼声，来源和 CC0 许可证记录在 `THIRD_PARTY_NOTICES.md`。`src/renderer/src/cat-sounds.ts` 按打招呼、触摸、提醒、开心和撒娇抗议选择不同录音；不再对录音变调，保留猫咪自然的声音。连续互动时会快速淡出上一声。运行时只读取本地 MP3，不访问网络。
